...
trait Tatum {
    ...
    public function tatumInit($output = null) {
        ...
        if($output['type'] == PaymentGatewayConst::TYPEADDMONEY) {
            try{
                $trx_id = $this->createTatumTransaction($output, $crypto_active_wallet);
            }catch(Exception $e) {
                throw new Exception($e->getMessage());
            }

            return redirect()->route('user.add.money.payment.crypto.address', $trx_id);
        }elseif($output['type'] == PaymentGatewayConst::TYPEDONATION){
             try{
                $trx_id = $this->createTatumTransaction($output, $crypto_active_wallet);
            }catch(Exception $e) {
                throw new Exception($e->getMessage());
            ...
            return redirect()->route('donation.payment.crypto.address', $trx_id);
        }
        ...
        if($output['type'] == PaymentGatewayConst::TYPEADDMONEY) {
            ...
            return [
                ...
                'address_info'      => [
                    ...
                    'submit_url'    => route('api.v1.user.add-money.payment.api.crypto.confirm',$trx_id)
                ],
            ];
        }elseif($output['type'] == PaymentGatewayConst::TYPEDONATION){
            try{
            ...
            return [
                ...
                'address_info'      => [
                    'coin'          => $crypto_asset->coin,
                    'address'       => $crypto_active_wallet->address,
                    'input_fields'  => $this->tatumUserTransactionRequirements(PaymentGatewayConst::TYPEDONATION),
                    'submit_url'    => route('api.v1.user.donation.payment.api.crypto.confirm',$trx_id)
                ],
            ];
        }
    ...
    public function createTatumTransaction($output, $crypto_active_wallet) {
    ...
    public function tatumUserTransactionRequirements($trx_type = null) {
        $requirements = [
            ...
            PaymentGatewayConst::TYPEDONATION => [
                [
                    'type'          => 'text',
                    'label'         =>  "Txn Hash",
                    'placeholder'   => "Enter Txn Hash",

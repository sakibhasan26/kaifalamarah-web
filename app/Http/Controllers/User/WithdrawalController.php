...
class WithdrawalController extends Controller
{
    ...
    {
        $page_title = __("Withdraw Moneny");
        $payment_gateways = PaymentGateway::has('currencies')->active()->moneyOut()->get();
        $money_transactions = Transaction::where('type', 'withdraw')->get();
        return view('frontend.pages.user.money_withdraw', compact('page_title','payment_gateways','money_transactions'));
    }

    public function index(Request $request)
    {
        $page_title = __('Withdraw Log');

    ...
    <div class="table-wrapper">
        <div class="table-header justify-content-end">
            <div class="table-btn-area">
                <a href="#about-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Service") }}</a>
            </div>
        </div>
                ...
                <tbody>
                    ...
                    @forelse ($data->value->items ?? [] as $key => $item)
                        <tr data-item="{{ json_encode($item) }}">
                            <td><i class="{{ $item->language->$system_default_lang->service_icon ?? "" }}"></i></td>
                            <td>{{ $item->language->$system_default_lang->heading ?? "" }}</td>
                            <td> {{ Str::limit($item->language->$system_default_lang->description ?? "", 100, '...')  }}</td>
                            <td>
...
{{-- Add Service --}}
<div id="about-add" class="mfp-hide large">
                            ...
                            <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif" id="modal-english" role="tabpanel" aria-labelledby="modal-english-tab">
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        ...
                                        'name'      => $default_lang_code . "_service_icon",
                                        'value'     => old($default_lang_code . "_service_icon"),
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.textarea',[
                                        ...
                                        'value'     => old($default_lang_code . "_description")
                                    ])
                                </div>
                            ...
                            @foreach ($languages as $item)
                                ...
                                <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="modal-{{ $item->name }}" role="tabpanel" aria-labelledby="modal-{{$item->name}}-tab">
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            'label'     => __("Service Icon")."*",
                                            'name'      => $lang_code . "_service_icon",
                                            'value'     => old($lang_code . "_service_icon"),
                                            'class'     => "form--control icp icp-auto iconpicker-element iconpicker-input",
                                        ])
                                    </div>
                        ...
                        <div class="tab-content" id="nav-tabContent">
                            ...
                            <div class="tab-pane @if (get_default_language_code() == language_const()::NOT_REMOVABLE) fade show active @endif" id="edit-modal-english" role="tabpanel" aria-labelledby="edit-modal-english-tab">
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        'label'     => __("Service Icon")."*",
                                        'name'      => $default_lang_code . "_service_icon_edit",
                                        'value'     => old($default_lang_code . "_service_icon"),
                                        'class'     => "form--control icp icp-auto iconpicker-element iconpicker-input",
                                    ])
                                </div>
                                <div class="form-group">
                                    @include('admin.components.form.input',[
                                        'label'     => __("Heading")."*",
                                        'name'      => $default_lang_code . "_heading_edit",
                                        'value'     => old($default_lang_code . "_heading")
                                    ])
                                </div>
                                ...
                                <div class="form-group">
                                    @include('admin.components.form.textarea',[
                                        'label'     => __("Details")."*",
                                        'name'      => $default_lang_code . "_description_edit",
                                        'value'     => old($default_lang_code . "_description_edit",$data->value->language->$default_lang_code->description ?? ""),
                            ...
                            @foreach ($languages as $item)
                                ...
                                <div class="tab-pane @if (get_default_language_code() == $item->code) fade show active @endif" id="edit-modal-{{ $item->name }}" role="tabpanel" aria-labelledby="edit-modal-{{$item->name}}-tab">
                                    ...
                                    <div class="form-group">
                                        @include('admin.components.form.input',[
                                            'label'     => __("Service Icon")."*",
                                            'name'      => $lang_code . "_service_icon_edit",
                                            'value'     => old($lang_code . "_service_icon"),
                                            'class'     => "form--control icp icp-auto iconpicker-element iconpicker-input",
                                        ])
                                    </div>
...
@push('script')
    ...
    <script>
        ...
        $(".edit-modal-button").click(function(){
            ...
            editModal.find("input[name="+default_language+"_service_icon_edit]").val(oldData.language[default_language].service_icon);
            ...
            editModal.find("textarea[name="+default_language+"_description_edit]").val(oldData.language[default_language].description);

            $.each(languages,function(index,item) {
                editModal.find("input[name="+item.code+"_service_icon_edit]").val((oldData.language[item.code] == undefined) ? "" : oldData.language[item.code].service_icon);

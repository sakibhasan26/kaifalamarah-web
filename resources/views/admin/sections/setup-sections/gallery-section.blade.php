...
@section('content')
        ...
        <div class="table-wrapper">
            <div class="table-header justify-content-end">
                <div class="table-btn-area">
                    <a href="#gallery-add" class="btn--base modal-btn"><i class="fas fa-plus me-1"></i> {{ __("Add Gallery Image") }}</a>
                </div>
            </div>
                    ...
                    <tbody>
                        ...
                        @forelse ($data->value->items ?? [] as $key => $item)
                            <tr data-item="{{ json_encode($item) }}">
                                ...
                                <td>
                                    {{ $item->language->$default_lang_code->title }}
                                </td>
                                <td>
                                    {{ $item->language->$default_lang_code->tag }}
                                </td>
    ...
    @include('admin.components.modals.site-section.gallery-add')

    {{-- Gallery Item Edit Modal --}}
    <div id="partner-edit" class="mfp-hide large">
        <div class="modal-data">
            <div class="modal-header px-0">
                <h5 class="modal-title">{{ __("Edit Gallery Image") }}</h5>
            </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-xl-6 col-lg-6 form-group">
                                                <div class="col-xl-12 col-lg-12 form-group">
                                                    ...
                                                    <select id="" name="{{ $lang_code }}_tag" class="form-control select2-auto-tokenize">
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
...
@push('script')
    ...
    <script>
        openModalWhenError("gallery-add","#gallery-add");
        openModalWhenError("partner-edit","#partner-edit");
        ...
        $(".edit-modal-button").click(function(){
            ...
            var editModal = $("#partner-edit");
            editModal.find("form").first().find("input[name=target]").val(oldData.id);
            editModal.find("input[name="+default_language+"_title]").val(oldData.language[default_language].title);

            var option = `<option value="${oldData.language[default_language].tag}" selected>${oldData.language[default_language].tag}</option>`;

            editModal.find("select[name="+default_language+"_tag]").html(option);
            editModal.find("select[name="+default_language+"_tag]").select2({
                tags:true,
            });

            editModal.find("input[name=image]").attr("data-preview-name",oldData.image);
            $.each(languages,function(index,item) {
                editModal.find("input[name="+item.code+"_title]").val((oldData.language[item.code] == undefined) ? '' : oldData.language[item.code].title);

                tag_data = (oldData.language[item.code] == undefined) ? '' : oldData.language[item.code].tag;
                var option_tag = `<option value="`+tag_data+`" selected>`+tag_data+`</option>`;
                editModal.find("select[name='"+item.code+"_tag']").html(option_tag);
            });

            fileHolderPreviewReInit("#partner-edit input[name=image]");
            openModalBySelector("#partner-edit");
        });
